...
async def show_add_user_page(request: Request):
    ...
    data_dict['student_id'] = utils.get_request_attribute(request, 'student_id', '')
    data_dict['year_course_reln_id'] = utils.get_request_attribute(request, 'year_course_reln_id', '')

    courses_by_year = course_year_dao.get_course_data_by_year_and_teacher(user_id, academic_year='2023-24')
...
async def save_user_course_data(request: Request, student_id: str = Form(...),
    ...
    df = user_course_dao.get_user_course_data_by_user_id_and_year_course_reln_id(student_id, year_course_reln_id)
    if len(df) > 0:
        data_dict['error_message'] = 'User is already enrolled in the course'
    else:
        # user is not enrolled in the course
        user_course_dao.insert_user_course(student_id, year_course_reln_id)
