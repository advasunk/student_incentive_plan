...
def get_all_courses_by_teacher_id(teacher_id):
    """Get all course data from the database"""
    sql = """    
        select c.subject, c.course_name, aycr.room_number, first_name || ' ' || last_name as teacher_name, aycr.*
        from student_incentive_plan.course_year_reln aycr
...
def get_course_data_by_year_date(year, course_id):
    ...
    results = pg_db_conn.execute_query("SELECT * FROM student_incentive_plan.course_year_reln "
                                       "WHERE academic_year = '{}' and course_id = {}"
                                       .format(year, course_id))
    return results
...
def insert_course(academic_year, course_id, room_number, teacher_id):
    """Insert course data into the database"""
    # check if course already exists
    df = get_course_data_by_year_date(academic_year, course_id)
    if len(df) > 0:
